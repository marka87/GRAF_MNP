...
int Test_Run_Demo(uint32_t z_encoder_start,
                  uint32_t z_encoder_end,
                  ad5684_dac_t *dac)
{
    // Speichert lokale Zustandsvariablen dauerhaft
    static uint8_t  init            = 1;     // Einmalige Init?
    static uint32_t start_time      = 0;     // Wann begann der Test?
    static int32_t  current_position= 0;
    static int32_t  direction       = 1;     // +1 nach oben, -1 nach unten

    // Wie lange soll DEMO laufen? 10 s
    const uint32_t duration_ms = 10000;

    // 1) Erstaufruf - Initialisierung
    if (init) {
        init = 0;
        start_time       = HAL_GetTick();   // Zeitstempel jetzt
        current_position = z_encoder_start; // Startwert
    ...
    if ((now - start_time) < duration_ms) {
        // Noch NICHT abgelaufen → eine kleine Schrittbewegung
        current_position += 350 * direction;

        if (current_position >= (int32_t)z_encoder_end) {
            current_position = z_encoder_end;
            direction = -1; // Richtung umkehren
        } else if (current_position <= (int32_t)z_encoder_start) {
            current_position = z_encoder_start;
            direction = 1; // Richtung umkehren
        }

        // Z-Achsen-Sollwert setzen
        Z_Axis_TargetPosition = (uint32_t) current_position;

        // Spannung ausgeben (z. B. 2.8V)
        ad5684_set_voltage(dac, 2.8f, z_mot);

        // Test läuft noch → Rückgabewert 0
        return 0;
    }
    else {
        // Zeit ist ABGELAUFEN → aufräumen
        init = 1;  // Beim nächsten Start wieder neu initialisieren
        current_position = z_encoder_start; // oder 0, je nach Wunsch
        direction = 1;

        // Motor neutral stellen
        ad5684_set_voltage(dac, TARGET_VOLTAGE_NEUTRAL, z_mot);

        // Rückgabewert 1 → Test ist fertig
        return 1;
    }
}
/* UART Callback */
...
//		Change_Z_Axis_TargetPosition_Stepwise(10);
		 ...
		 if (current_state == IDLE_START) {
		        ...
		        if (!byte_handled) {
		    	 ...
		    	 // Rufe Demo-Test auf
		    	        int done = Test_Run_Demo(z_encoder_start, z_encoder_end, &dac);
		    	        if (done == 1) {
		    	            // Test fertig -> next state
