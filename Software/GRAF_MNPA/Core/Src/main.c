...
typedef enum {IDLE_START, EXEC_REFERENCE_RUN, BLA} run_state_t;
...
//	ADC_Init(&hadc1);
  ...
  /* USER CODE BEGIN WHILE */
	while (1) {
		...
		else if (current_state == BLA) {

		}


		//Change_Z_Axis_TargetPosition_Stepwise(50);
		...
		if (HAL_GetTick() >= next_10ms_tick) {
			...
			// Handle button presses (debouncing)
			handle_button_ok();
		}
