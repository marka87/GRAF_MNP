...
//void Change_Z_Axis_TargetPosition_Stepwise(void);
...
//	ADC_Init(&hadc1);
	...
	while (1) {
		...
		// Anzeige für Startzustand

		if (current_state == IDLE_START) {
//			display_jazz_write_string_5x7(&display1, 0, "Zum Referenzieren:");
//			display_jazz_write_string_5x7(&display1, 1, "PC: s, Taster: OK");
//			display_jazz_write_string_5x7(&display1, 7, "      OK      ");
			if (btn_ok_pressed) {
				btn_ok_pressed = 0;
				current_state = EXEC_REFERENCE_RUN;
			}

			if (byte_handled == false && (byte_received == 's')) {
				byte_handled = true;
				current_state = EXEC_REFERENCE_RUN;
			}
			...
			if (z_axis_success) {
				char msg[] = "Z Reference Run successful\r\n";
...
//			display_jazz_clear(&display1);
//			display_jazz_write_string_5x7(&display1, 0, "DEMO (100): 1");
//			display_jazz_write_string_5x7(&display1, 1, "KURZ (1000): 2");
//			display_jazz_write_string_5x7(&display1, 2, "LANG (10000): 3");
//			display_jazz_write_string_5x7(&display1, 8, "1     2     3");
//
...
//			if (byte_handled == false && (byte_received == '1')) {
//				byte_handled = true;
//				mode = 0; // DEMO-Modus
////									current_state = TEST_RUN;
//			} else if (byte_handled == false && (byte_received == '2')) {
//				byte_handled = true;
//				mode = 1; // KURZ-Modus
////									current_state = TEST_RUN;
//			} else if (byte_handled == false && (byte_received == '3')) {
//				byte_handled = true;
//				mode = 2; // LANG-Modus
////									current_state = TEST_RUN;
//			}
//
////			Z_Axis_Test(&dac, mode);
//
//			current_state = TEST_RUN;
//
//			} else if (current_state == TEST_RUN) {
//
////			Z_Axis_Test_NonBlocking(&dac);
//
...
//			HAL_Delay(2000);
		...
		if (HAL_GetTick() >= next_100ms_tick) {
			next_100ms_tick = HAL_GetTick() + 10;
			handle_button_ok();
			handle_button_up();
			handle_button_down();
		}
